...
class Pdb {
    ...
    static get KEY_SEP() { return "х" };//U+0425
...
namespace Pdb {
    export class ManageTable {
        ...
        deleteTableKeys(tableName: string) {
    ...
    export class Database {
        ...
        isPrimaryKeyEmpty(tableName: string, targetObj) {
        ...
        _createKey(tableName: string, data) {
            var pkArray = this.managedTable.getTableKeys(tableName);
            var s = "";
            for (var i = 0; i < pkArray.length; i++) {
                if (i !== 0) s += Pdb.KEY_SEP;
                s += data[ pkArray[i] ];
            }
            return s;
        }

        _createRecord(tableName: string, data) {
        ...
        openDatabase(dbName: string) {
            ...
            var dfd = new Promise(function (resolve, reject) {
                ...
                openReq.onsuccess = function (event: IDBOpenRequestEvent) {
                    _self._db = event.target.result;
        ...
        isExistTable(tableName: string) {
            var list = this.getTableList();
            for (let i = 0, len = list.length; i < len; i++) {
                if (list[i] === tableName) return true;
            }
            return false;
        }

        createTable(tableName: string, primaryKeys: Array<string>) {
            var _self = this;
            var dfd = new Promise(function(resolve, reject) {
        ...
        dropTables(tableNames: Array<string>) {
            var _self = this;
            var dfd = new Promise(function(resolve, reject) {
        ...
        importData(tableName: string, datas: Array<any>) {
            var _self = this;
            var dfd = new Promise(function(resolve, reject) {
        ...
        update(tableName: string, data) {
            ...
            var dfd = new Promise(function(resolve, reject) {
                ...
                tx.objectStore(tableName)
                    .get(_self._createKey(tableName, data)).onsuccess =  function(ue: IDBOpenRequestEvent) {
        ...
        delete(tableName: string, keyData) {
            var _self = this;
            var dfd = new Promise(function(resolve, reject) {
        ...
        getTableData (tableName: string) {
            ...
            var dfd = new Promise(function(resolve, reject) {
                ...
                table.openCursor().onsuccess = function (event: IDBOpenCursorEvent) {
    ...
    export class Query {
        ...
        static qdbJoin(db: Array<object>, table: Array<object>, aliasName: string,
            ...
            return Pdb.Array$map(db, function (row) {
                row[aliasName] = Pdb.Array$map(table, function (one) {
                    ...
                    if (whereFunc(condRow, params, query)) return one;
                });
                return row;
            });
        }
         ...
         */
        private static _getValue(columnName: string, source) {
            if (!source) return undefined;
            var c = columnName.split('.');
            var v = source;
            for (var i = 0, len = c.length; i < len; i++) {
                if (!v[c[i]]) return undefined;
                else v = v[c[i]];
            }
            return v;
        }

        static count(array: Array<any>) {
            return array.length;
        }

        static max(array: Array<any>, key: string) {
            var get = Pdb.Query._getValue;
            var c = 0;
            if (array.length == 1) return get(key, array[0]);
        ...
        static min(array: Array<any>, key: string) {
            var get = Pdb.Query._getValue;
            var c = 0;
            if (array.length == 1) return get(key, array[0]);
            return array.reduce(function (x, y, currentIndex) {
        ...
        static average(array: Array<any>, key: string) {
         ...
         * SQL "from" phrase.
         */
        from(tableName: string, aliasName: string) {
         ...
         * SQL "from" phrase.
         */
        exists(whereFunc: Function, params): boolean {
         ...
         * SQL "where" phrase.
         */
        orderBy(orderArray: Array<any>, params) {
         ...
         * SQL "select" phrase.
         */
        select(columnFunc: Function, params) {
            var tbl = this._tableObj;
            var array = [];
            Pdb.Array$each(this.db, function (row, index) {
        ...
        private _select_(aliasName: string) {
    ...
    export class QueryBuilder {
        databaseName: string;
        queryQueue: Array<PdbQueryQuere>;
        ...
        with(tableName: string, withFn: Function) {
        ...
        leftJoin(tableName: string, aliasName: string, whereFunc: Function, isInnerQuery?: boolean) {
            this.queryQueue.push({
                type: "leftJoin",
        ...
        innerJoin(tableName: string, aliasName: string, whereFunc: Function, isInnerQuery?: boolean) {
            this.queryQueue.push({
                type: "innerJoin",
                ex: { tableName: tableName, aliasName: aliasName, fn: whereFunc.toString(), isInnerQuery: isInnerQuery }
            });
        ...
        where(whereFunc: Function, isInnerQuery?: boolean) {
            this.queryQueue.push({
                type: "where",
    ...
    export class Async {
        static post(libUrl: string, parameter: PdbBuildParameter) {
            return new Promise(function (resolve, reject) {
    ...
    export class WebWorker {
        static async execute(parameters: PdbBuildParameter) {
            ...
            for (var i = 0, len = queryQueue.length; i < len; i++) {
                ...
                try {
                    ...
                    fn = queue.ex.fn ? Function(
                        queue.ex.fn.substring(queue.ex.fn.indexOf("(") + 1, queue.ex.fn.indexOf(")")),
                        queue.ex.fn.substring(queue.ex.fn.indexOf("{") + 1, queue.ex.fn.lastIndexOf("}") - 1)
                    ) : null;
                } catch (e) {
